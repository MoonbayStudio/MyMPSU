...
MSI использует `bundle.windows.wix.language: "ru-RU"`: эта локаль выбирает
кодовую страницу Windows-1251 для кириллического названия «Мой МПГУ». Значение
WiX по умолчанию `en-US` использует Windows-1252 и не подходит для этих строк.
CI запускает упаковщик с `--verbose`, чтобы ошибки WiX/NSIS были видны в логе.
Настройка локали MSI не меняет системные значки Apple и сборки macOS/Linux.
